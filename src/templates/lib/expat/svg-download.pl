...
my $chart_type = param('svg_chartType')
	or die "Missing 'chart' parameter";
die "Invalid chart value"
	unless $chart_type eq "heatmap" || $chart_type eq "linegraph";
...
if ($output_format eq "svg") {
	...
	print header(-type=>"image/svg+xml",
		     -attachment=>"lotus_expressionAtlas_".$chart_type."_".$dt->ymd."_".$dt->hms.".svg");
	print $data;
	exit(0);
}
## PDF/PNG output
