<?php
	...
	try {
		...
		if(!empty($_GET['code']) && !empty($_GET['state']) && !empty($_SESSION['oauth2_state'])) {
			...
			throw new Exception('We have encountered an error (code '.$_GET['error'].') with LinkedIn OAuth2 interface'.(isset($_GET['error_description']) && !empty($_GET['error_description']) ? ': '.$_GET['error_description'] : '.'));
		}
	} catch(Exception $e) {
