		...
		<div id="account-tabs">
			...
			<?php
				try {
					...
					$orgs = array(
						'Select a pre-existing organization, or enter a new one' => array('value' => ''),
						'No affiliation' => array('value' => '')
						);
					$o = $db->prepare('SELECT Organization AS Organization, COUNT(*) AS UserCount FROM auth GROUP BY Organization ORDER BY Organization');
					$o->execute();
					while($rows = $o->fetch(PDO::FETCH_ASSOC)) {
						$orgs[$rows['Organization']] = array('value' => $rows['Organization'], 'userCount' => $rows['UserCount']);
					}
							...
							<div class="col-two">
								<select name="organization" id="organization">
									<?php
										...
										foreach($orgs as $title => $data) {
											$orgs_opts[] = '<option value="'.$data['value'].'" '.($userData['Organization'] === $title ? 'selected' : '').'>'.$title.(!empty($data['userCount']) ? ' ['.$data['userCount'].(pl($data['userCount'], ' user')).']' : '').'</option>';
										}
					...
					<ul>
						...
						<li>remove any associated third party OAuth IDs, if any, with <em>Lotus</em> Base,</li>
						<li>unsubscribe you from the <em>Lotus</em> Base mailing list, and</li>
					...
					<ul>
						<li>remove <em>LORE1</em> orders placed by a user</li>
					</ul>
